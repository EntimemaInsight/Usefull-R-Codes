...
woe_character_macro = defmacro(Sample_application, Sample_application_ext, VarName, 
  ...
  # VarName <- "Last_InsuranceType_combined"
  # minBinSize1 <- 0.05
  ...
  Sample_application_ext <- Sample_application_ext%>%
    mutate(Variable = case_when(is.na(Variable) ~ "Missing", 
                                ...
                                TRUE ~ Variable))
  Variable_sample <- Sample_application%>% 
  ...
  names <- as.matrix(Variable_sample [,1])
  
  Variable_sample <- data.matrix(Variable_sample)[,-1]
  rownames(Variable_sample) <-names
  #rownames(Variable_sample) <- Variable_sample [,1]
  # 
  # for (i in seq_along(1:nrow(Variable_sample))) {
  #   if (i == 1){ 
  #     Variable_sample [i, "Bin"]= 1
  #   } else if(Variable_sample[i, "Population"] >= minBinSize1){
  #     Variable_sample [i, "Bin"] = as.numeric(Variable_sample [i-1, "Bin"])+1
  #   } else  Variable_sample [i, "Bin"] = as.numeric(Variable_sample [i-1, "Bin"])
  # }
  ...
  #     Variable_sample [i, "Bin"]= 1
  #   }
  #   else if(i != 1 & Variable_sample [i-1, "Bin"]== 1 & Variable_sample[i-1, "Population"] <= minBinSize1) {
  #      Variable_sample [i, "Bin"]= 1}
  #   else
  #     if(Variable_sample[i, "Population"] >= minBinSize1){
  #       Variable_sample [i, "Bin"] = as.numeric(Variable_sample [i-1, "Bin"])+1
  #     } else  Variable_sample [i, "Bin"] = as.numeric(Variable_sample [i-1, "Bin"])
  # }
  ...
  for (i in seq_along((1:nrow(Variable_sample)))){
    ...
    } else if (i != 1 & Variable_sample[i-1, "CumSum"] >= minBinSize1)
    { if (Variable_sample[i, "Rest"] >= minBinSize1){
      Variable_sample [i, "Bin"] = as.numeric(Variable_sample [i-1, "Bin"])+1
      Variable_sample[i, "CumSum"] = Variable_sample[i, "Population"] 
    } else {
  ...
  # Variable_sample2 <- Variable_sample1 %>%  select(Variable, Bin, WoE_Bin)
  # colnames(Variable_sample2)[colnames(Variable_sample2) == "Variable" ]  <- VarName
  
  Sample_application_ext <- Sample_application_ext %>% left_join(Variable_sample1 %>%   dplyr::select(Variable, Bin, WoE_Bin), by = "Variable")

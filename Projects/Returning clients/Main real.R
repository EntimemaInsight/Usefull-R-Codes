tryCatch(
  expr = {
    ...
    #input_data <- readxl::read_xlsx('./local_test_file_ith_JSON2.xlsx')
    ...
    if(NewClientCheck == 1){
      ...
      TimeT <- ifelse(is.na(TimeT), -999999999, TimeT)
      ...
      if(TimeT >= 0){
        ...
        data <- data %>%
          ...
          dplyr::filter(SumaP <= MaxOffer,
                        SumaP >= MinOffer) %>%
          dplyr::left_join(ProfitsRefinancing %>% select(RiskClass, ProfitCreditDailyPercent), by = "RiskClass") %>%
          dplyr::rename(ExpectedProfitPercent = ProfitCreditDailyPercent) %>%
          dplyr::mutate(ExpectedProfit = SumaP * ExpectedProfitPercent) %>%
          dplyr::arrange(desc(ExpectedProfit)) %>%
          dplyr::slice_head()
        ...
        scoringClass <<- ""
        scoringDecision <<- "Allowed"
        #maxAcceptableCreditSum <<- OriginalPD
